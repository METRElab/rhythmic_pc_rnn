...
class BeatPCRNN(nn.Module):
    def __init__(
        ...
        # Initialize prediction weights for beat pathway
    ...
    def compute_prediction_errors(self, beat_input):
        """Compute prediction errors for all units and inputs."""
        mu_rec, mu_b = self.compute_predictions()
    ...
    def compute_free_energy(self, beat_input: Optional[torch.Tensor]) -> torch.Tensor:
        """Compute free energy (prediction error)."""
        e_rec, e_b = self.compute_prediction_errors(beat_input)
        return 0.5 * (torch.sum(e_rec ** 2) + e_b ** 2)

    def update_weights(self, e_rec: torch.Tensor, e_b: torch.Tensor):
        """Update weights based on prediction errors."""
        # Compute weight updates
